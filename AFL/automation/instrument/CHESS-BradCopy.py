...
class CHESSID3B(ScatteringInstrument,Driver):
    ...
    def lastTransmission(self,set_empty_transmission=False,return_full=False):
        # need to change out where these numbers come from
    ...
    def setFilename(self,name):
        ...
        # self.client.run_cmd(f'newfile {name}')
        # possible adaption
        self.client.run_cmd(pye.caput("IN:LARMOR:DAE:TITLE:SP", f"{name}"))
        ...
        try:
            ...
            while nattempts<11:
                ...
                try:
                    ...
                    break
        # returns numpy array with 0s if there was any NaNs
    ...
    # 1 of 2 main things we care about
    def _simple_expose(self,name=None,exposure=None,block=False):
        # set/get file name for exposure
        if name is None:
        ...
        # set status text to say what exposure we are running
        self.status_txt = f'Starting {exposure} s count named {name}'
        # add this to the app debugger as well
        # this is a flask server that this is sitting in; shouldn't need to change
        if self.app is not None:
            self.app.logger.debug(f'Starting exposure with name {name} for {exposure} s')
        # send instrument commands to the instrument - we will need to adjust these
        # note, open/close are probably just CHESS dependent
        self.client.run_cmd(f'opens') # open shutter
        self.client.run_cmd(f'tseries 1 {exposure}') # expose for {exposure} seconds
        self.client.run_cmd(f'closes') # close shutter
        # do we want to prevent this function from moving forward if something else is not implemented?
        # if so, what do we do? Appears to be not implemented currently
        if block:
    ...
    # 2 of 2 main things we care about
    def expose(self,name=None,exposure=None,nexp=1,block=True,reduce_data=True,measure_transmission=True,save_nexus=True):
        ...
        if exposure is None:
            ...
            self.setExposure(exposure)
        # print that we are starting the exposure
        self.status_txt = f'Starting {exposure} s count named {name}'
        if self.app is not None:
            self.app.logger.debug(f'Starting exposure with name {name} for {exposure} s')
        # open some empty lists...what are these?
        pos_trans = []
        pos_raw_data = []
        pos_simple_trans = []
        pos_reduced_data = []
        # enumerate through measurement positions, taking multiple measurement position
        # save all data, but move forward with lowest transmission
        for idx,pos in enumerate(self.config['measurement_positions']):
            self.client.run_cmd(f'umv samx {pos}') # command to tell CHESS/beamline sample motor where to move to 
            self.client.run_cmd(f'opens') # open shutter, don't really need for not CHESS
            self.client.run_cmd(f'tseries {nexp} {exposure/10}') # low exposure time measurement
            self.client.run_cmd(f'tseries {nexp} {exposure}',block=True) # high exposure time measurement
            self.client.run_cmd(f'closes') # close shutter, don't really need for not CHESS
            
            pos_raw_data.append(self.getData()) # grab raw data from the instrument and add it to the list of data
            pos_reduced_data.append(self.getReducedData(write_data=True,filename=f'{name}_{idx}')) # grab the data that has been reduced and add it to the list
            pos_trans.append(self.lastTransmission(return_full=True)) # essentially gets the last measured beam instensity for this run
                                                                      # contains much more meta data with it
            pos_simple_trans.append(pos_trans[-1][0]) # grabs the single transmision number we actually want for each exposure
                
        #time.sleep(0.5)
        if block or reduce_data or save_nexus:
            chosen_id = np.argmin(pos_simple_trans) # grabs the index of the lowest transmission exposure, assuming less exposure means most sample tested
            ...
            data = pos_raw_data[chosen_id] # sets data to be the chosen, lowest transmission sample
