...
class Jubilee_Driver(Driver):
    defaults = {}
    defaults['machine_address'] = '/dev/ttyACM0'
    defaults['pipette_tool_id'] = 0
    defaults['pipette_tool_name'] = 'OT2_P300'

    def __init__(self,overrides=None):
        self.app = None
        Driver.__init__(self,name='Jubilee_Driver',defaults=self.gather_defaults(),overrides=overrides)
        self.name = 'Jubilee_Driver'
        #self.protocol = opentrons.execute.get_protocol_api('2.0')
        self.machine = Machine(address = self.config['machine_address'])
        #instantiate science_jubilee object reprs of tools
    ...
    def pickup_tool(self,tool):
        '''
        Load a tool onto the head by name (case-insensitive match)
        '''
        try:
            tool = machine.tools[int(tool)]
        except ValueError:
            tool = [k['tool'] for k in machine.tools.values() if k['name'].lower() == tool.lower()][0]
        self.machine.load_tool(tool)
    ...
    def status(self):
        ...
        for k,v in self.machine.tools.items():
            status.append(f'Tool slot {k}: {v.name}')
            
            if isinstance(v['tool'],science_jubilee.tools.Pipette):
            ...
            elif isinstance(v['tool'],science_jubilee.tools.Camera):
                pass
            # insert logic to handle other tools here
        for k,v in self.machine.deck.slots.items():
            status.append(str(v))
        return status
