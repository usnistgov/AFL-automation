...
class CastingServer(Driver):
    ...
    def __init__(self,
        ...
        if not (len(prep_url.split(':'))==2):
            raise ValueError('Need to specify both ip and port on prep_url')
    ...
    def log_event(self,manifest_key,event,write=True):
        ...
        for k,v in event.items():
            if k=='mix_before' or k=='mix_after':
                v = str(v)
            ds_event[k] = ('event',[v])
    ...
    def assign_targets(self,protocols,target_map=None):
        if target_map is None:
            ...
            for t in self.targets:
                response = self.prep_client.enqueue(task_name='get_prep_target',interactive=True)
                if isinstance(response, dict) and 'return_val' in response:
                    prep_target = response['return_val']
    ...
    def prepare_casting_stocks(self,**spec):
        ...
        for sample_name,task in zip(sample_names,protocol):
            ...
            response = self.prep_client.transfer(interactive=True,**task)
            self.last_prep = {}
            if isinstance(response, dict):
                self.last_prep.update(response)
                self.last_prep_time = response.get('ended')
    ...
    def prepare_and_cast(self,**sample):
        ...
        min_mix_time = sample.get('min_mix_time', 0)  # Get min_mix_time from sample dict, default to 0
        ...
        for task in protocols['prep_protocol']:
            ...
            if isinstance(response, dict):
                # self.last_prep.update(response)
                self.last_prep_time = response.get('ended')
                self.last_prep['queued'] = response.get('queued')
                self.last_prep['started'] = response.get('started')
                self.last_prep['ended'] = response.get('ended')
            
            self.last_prep['event_type'] = 'prep'
            self.last_prep['sample_name'] = sample_name
            self.last_prep['min_mix_time'] = datetime.timedelta(seconds=min_mix_time)
            self.last_prep['actual_mix_time'] = datetime.timedelta(seconds=0)
            self.last_prep.update(task)
            self.log_event('manifest',self.last_prep)
            
        # shouldn't have to wait, but let's do this for safety
        if self.prep_uuid is not None: 
            self.prep_client.wait(self.prep_uuid)

        # Add mixing wait time if specified
        if min_mix_time > 0:
            mix_start_time = datetime.datetime.now()
            self.update_status(f'Waiting {min_mix_time} seconds for mixing of sample: {sample_name}')
            time.sleep(min_mix_time)
            mix_end_time = datetime.datetime.now()
            actual_mix_time = (mix_end_time - mix_start_time).total_seconds()
            
            # Log the mixing event
            mix_event = {
                'event_type': 'mix',
                'sample_name': sample_name,
                'min_mix_time': datetime.timedelta(seconds=min_mix_time),
                'actual_mix_time': datetime.timedelta(seconds=actual_mix_time),
                'started': mix_start_time,
                'ended': mix_end_time,
                'queued': mix_start_time
            }
            self.log_event('manifest', mix_event)

        self.update_status(f'Casting sample: {sample_name}')
        last_plate_name = None
        for plate_name,task in zip(plate_names,protocols['cast_protocol']):
            response = self.prep_client.transfer(**task,interactive=True)
            self.last_prep = {}
            if isinstance(response, dict):
                # self.last_prep.update(response)
                self.last_prep['queued'] = response.get('queued')
            ...
            self.last_prep.update(task)
            self.log_event('manifest',self.last_prep)
            self.update_status(f'Cast {sample_name}!')
            last_plate_name = plate_name
            
        self.update_status(f'All done for sample {sample_name} on plate {last_plate_name}!')
...
_DEFAULT_CUSTOM_CONFIG = {
        ...
        'manifest': '/home/afl642/',
        'manifest_prep': '/home/afl642/',
        'manifest_cast': '/home/afl642/'
}
