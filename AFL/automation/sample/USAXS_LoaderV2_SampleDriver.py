...
class USAXS_LoaderV2_SampleDriver(Driver):
    defaults={}
    ...
    def __init__(self,
        ...
        Driver.__init__(self,name='USAXS_SampleDriver',defaults=self.gather_defaults(),overrides=overrides)
        ...
        #load sensor
        self.sensor_client = Client(sensor_url.split(':')[0],port=sensor_url.split(':')[1])
        self.sensor_client.login('SampleServer_SensorClient')
        self.sensor_client.debug(False)
    ...
    def measure(self,sample):
        ...
        sas_uuid = self.sas_client.enqueue(task_name='expose',name=sample['name'],block=True)
    ...
    def process_sample(self,sample):
        ...
        if self.rinse_uuid is not None:
            ...
            self.update_status(f'Calibrating sensor!')
            self.sensor_client.enqueue(task_name='calibrate_sensor')#no need to wait, should be fast
