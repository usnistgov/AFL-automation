...
class SAS_SampleDriver(Driver):
    ...
    def measure(self,sample):
        ...
        if wiggle:
            wiggle_step = sample.get('wiggle_step',1/3)
            sample_volume = sample['volume']

            #ensure we are at the load speed
            load_speed = self.load_client.get_config('load_speed',interactive=True)['return_val']
            self.load_client.enqueue(task_name='setRate',device='pump',rate=load_speed)

            'wiggle_params = [step_multiple,source,dest]'
            wiggle_params = []
            wiggle_params.append([2,'air','cell']) #net position = +2/3
            wiggle_params.append([4,'cell','waste']) #net position = -2/3
            # wiggle_params.append([3,'air','cell']) #net position = +1/3
            # wiggle_params.append([2,'cell','waste']) #net position = -1/3
            for step_multiple,source,dest in wiggle_params:
                vol = step_multiple*wiggle_step*sample_volume 
                load_uuid = self.load_client.enqueue(task_name='transfer',source=source,dest=dest,vol_source=vol)
                self.load_client.wait(load_uuid)
