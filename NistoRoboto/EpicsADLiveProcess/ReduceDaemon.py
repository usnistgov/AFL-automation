...
class ReduceDaemon(threading.Thread):
    ...
    def run(self):
        while not self._stop:
            ...
            res1d = self.integrator.integrate1d(data[2],self.npts,unit='q_A^-1',mask=np.zeros(np.shape(data[2])),method='csr_ocl',error_model='azimuthal')
            res2d = self.integrator.integrate2d(data[2],self.npts,unit='q_A^-1',mask=np.zeros(np.shape(data[2])),method='csr_ocl')
