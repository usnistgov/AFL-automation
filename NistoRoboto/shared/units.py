...
def is_molarity(value):
    return ((len(value.dimensionality)==2) and (value.dimensionality['[length]']==-3) and (value.dimensionality['[substance]']==1))

def is_mass(value):
    return ((len(value.dimensionality)==1) and (value.dimensionality['[mass]']==1))

def is_density(value):
    return ((len(value.dimensionality)==2) and (value.dimensionality['[mass]']==1) and (value.dimensionality['[length]']==-3))
...
def enforce_units(value,unit_type=None):
    '''Ensure that a number has units and convert to the default_units'''
    # None bypasses all unit testing
    if value is None:
        return
    
    if (unit_type is not None) and (unit_type.lower() not in supported_types):
