...
def test_massbalance_driver_mixed_solvents_mass():
    ...
    # Add stocks
    mb.add_stock({"name": "Stock1", "masses": {"H2O": f"20 g"}, "location": "1A1"})
    mb.add_stock({"name": "Stock2", "masses": {"Hexanes": f"20 g"}, "location": "1A2"})
    mb.add_stock(
        {
            "name": "Stock3",
            "masses": {"H2O": f"20 g"},
    ...
    for i, result in enumerate(mb.balanced):
        ...
        assert sub_balanced.mass_fraction["H2O"] == pytest.approx(
            sub_target.mass_fraction["H2O"]
        )
        assert sub_balanced.mass_fraction["Hexanes"] == pytest.approx(
            sub_target.mass_fraction["Hexanes"]
        )
